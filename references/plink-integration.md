...
| 编写业务层 PLink 手写实体（不修改框架或编辑器工具） | `.cursor/skills/plink-framework/SKILL.md` 及其要求的资料 |
...
- 同名、同路径或文件存在不足以证明可复用。核对实际切图内容（含尺寸、透明区域）及影响显示的导入设置（如 Sprite / Texture 类型、切片与九宫格边界、PPU、压缩）；能证明与当前 PSD 的预期输出一致才复用。缺失、已变化或暂不能证明一致的图片由现成工具按当前正式输出配置处理，核对本次写入范围与实际结果，不自制切图。
- 已有 UI 先保留正式资源的可恢复版本、GUID/引用和当前两份说明中的适配基线，然后按新建流程生成正式资源并恢复仍适用适配；不默认新建候选模块。正式图片更新、导入设置及 Prefab 引用变化纳入本次范围，已有明确授权不额外确认。共享图片仅当前页面需要变化时，优先通过工具现有能力生成页面专用资源；确需更新共享资源时先查清引用范围，超出原授权的影响单独说明。
...
完整制作流程在 Prefab 减法和布局完成后执行本节；已有指定阶段边界时按用户范围处理。必须使用现成 XUGUI 代码生成与刷新工具，面板、Item、Mono、Auto 及初始骨架由工具创建；不自行实现另一套模板写出、生成扫描或绑定赋值逻辑，外部调用代码也不例外。随后仅维护已有业务及新增视觉节点接入；完全未实现的功能交给另一个 Skill。

维护任务按 [已有链条维护](existing-ui-maintenance.md) 区分：每次 PSD 重新生成正式 Prefab 并恢复适配后都调用本节工具流程；用户仅调整适配时检查显示、维护原说明，不运行生成或刷新。[仅修改手写功能代码](existing-ui-maintenance.md#仅修改手写功能代码) 且节点、绑定组件与序列化引用需求均未变化时，不运行生成或刷新。已有完整流程授权时，对范围内的适配恢复、绑定及业务功能代码不再二次确认。
...
先传 `true` 生成，依据新生成的字段与类型完成已确认范围内必要的手写引用迁移、删除引用处理或类型适配，待 Unity 编译通过且生成类型可加载，再传 `false` 刷新。没有引用变化则跳过修复；不能因旧手写代码编译失败而等待绑定通过后才修，也不修改生成文件来恢复旧字段。其余已有功能的差异接入在绑定通过后完成，清单内修复不二次确认。`finalPanelCsDirectory` 是必须已存在的最终面板代码目录，静态方法不会追加面板名；UI 操作窗口选择的是代码父目录，由窗口追加面板名。当前 `RequestScriptCompilation()` 内实际编译请求被注释，不能依赖方法名或一次刷新调用自动等待新类型编译。
...
项目同时安装 XUGUIManager.Extension 与 DOTween 时遵循当前 Skill 的 AddClickListener 注册规则，未启用时不引入该依赖。本流程只维护业务逻辑及界面资源，不修改框架或编辑器工具；查源码用于确定调用和行为，不代表获得修改权限。
