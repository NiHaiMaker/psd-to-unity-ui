...
两步生成是硬性约束：**`_UI.psd / _UI.psb → Prefab` 必须走项目现成 PSD 工具，`Prefab → 代码与绑定` 必须走项目现成 UI 生成/刷新工具。** 自动化只能调用菜单、按钮或其同一方法入口，不得另写生成器、拼接底层导出服务或手写生成结果；脚本放在 Assets 外也不例外。输出沿用当前工具配置及已确认参数，工具失败时有界恢复并报告实际状态，不自行生成替代产物。
...
1. **保留源文件并整理 PSD。** 默认设计画布为 750×1334，另存 `<原名>_UI.psd`；重名时使用时间戳，PSB 保留扩展名。自身或祖先组关闭小眼睛的图层默认不进入整理输出，用户明确要求保留时才例外；排除项写入原 PSD 修改说明。根据用途整理命名、分组、绑定标记及候选 Item。只改名任务保留原尺寸和结构，直接导出人工修正版不自动清理其隐藏层。
